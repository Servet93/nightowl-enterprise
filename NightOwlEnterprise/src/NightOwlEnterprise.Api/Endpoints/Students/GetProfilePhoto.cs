...
public static class GetProfilePhoto
{
    ...
    {
        endpoints.MapGet("/{studentId}/profile-photo", async Task<Results<EmptyHttpResult, ProblemHttpResult>>
            ([FromRoute] Guid studentId, HttpContext httpContext, [FromServices] IServiceProvider sp) =>
        {
            var dbContext = sp.GetRequiredService<ApplicationDbContext>();

            var profilePhoto = dbContext.ProfilePhotos.FirstOrDefault(x => x.UserId == studentId);
        ...
        }).ProducesProblem(StatusCodes.Status400BadRequest).WithOpenApi().WithTags(TagConstants.CoachMeStudents);
    }
}