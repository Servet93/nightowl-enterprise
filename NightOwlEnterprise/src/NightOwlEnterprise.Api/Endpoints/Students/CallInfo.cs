...
public static class CallInfo
{
    ...
    {
        endpoints.MapGet("/call-info", async Task<Results<Ok<CallInfoResponse>, ProblemHttpResult>>
            (ClaimsPrincipal claimsPrincipal, [FromServices] IServiceProvider sp) =>
        {
            ...
            var invitations = dbContext.Invitations.Where(x => x.StudentId == studentId).OrderByDescending(x => x.Date)
                .Take(2).Select(x => new
                {
                    Id = x.Id,
                    Date = x.Date,
            ...
            {
                callInfoResponse.VideoCall = new Call()
                {
                    Id = videoCall.Id,
                    Date = videoCall.Date,
            ...
            {
                callInfoResponse.VoiceCall = new Call()
                {
                    Id = voiceCall.Id,
                    Date = voiceCall.Date,
    ...
    {
        public Call VideoCall { get; set; }
        
        public Call VoiceCall { get; set; }
    }
